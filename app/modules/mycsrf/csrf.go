...
func CsrfFilter(c *revel.Controller, fc []revel.Filter) {
	...
	if !allowedMethods[c.Request.Method] && !IsExempt(c) {
		...
		if requestToken == "" || !compareToken(requestToken, token.(string)) {
...
func sameOrigin(u1Host string, u2 *url.URL) bool {
	//revel.AppLog.Info("url:%s,u1.Scheme:%s, u2.Scheme:%s, u1.Host:%s, u2.Host:%s", u1.Path, u1.Scheme, u2.Scheme, u1.Host, u2.Host)
	//revel.AppLog.Info("u1Host:%s,u2Host:%s", u1Host, u2.Host)
